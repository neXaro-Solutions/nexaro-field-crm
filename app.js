const KEY='nexaro-crm-v4';
let S=JSON.parse(localStorage.getItem(KEY)||localStorage.getItem('nexaro-crm-v3')||localStorage.getItem('nexaro-crm-v2')||localStorage.getItem('nexaro-crm-v1')||'{"leads":[]}');
let filter='all';
...
const SUMUP={
  ...
  payg:{
    monthly:'0 €',
    rate:'1,39 %',
    note:'Umsatzbasiertes Zahlen; keine monatliche Grundgebühr.'
  },
  plus:{
    monthly:'19 €',
    yearly:'199 €',
    rate:'0,79 %',
    note:'Für Vor-Ort-Zahlungen mit inländischen/EWR-Verbraucherkarten; andere Karten können 1,39 % kosten.'
  },
  products:[
    {
      name:'Tap to Pay',
      price:'0 € Hardware',
      fit:'Kontaktlose Zahlungen direkt mit kompatiblem Smartphone',
      url:'https://www.sumup.com/de-de/tap-to-pay/'
    },
    {
      name:'Solo Lite',
      price:'34 €*',
      fit:'Kompaktes Kartenlesegerät, mit Smartphone gekoppelt',
      url:'https://www.sumup.com/de-de/solo-lite-kartenterminal/'
    },
    {
      name:'Solo',
      price:'79 €*',
      fit:'Eigenständiges Kartenterminal für mobile und stationäre Nutzung',
      url:'https://www.sumup.com/de-de/solo-kartenlesegeraet/'
    },
    {
      name:'Terminal',
      price:'169 €*',
      fit:'Eigenständiges All-in-one-Gerät mit Bestell- und Kassenfunktionen',
      url:'https://www.sumup.com/de-de/terminal-kartenterminal/'
    }
  ]
};

function sumupTariffHtml(){
  return `<div class="tariff-card">
    <div class="assistant-label">SUMUP DEUTSCHLAND 🇩🇪 · TARIF-CHECK</div>
    <h3>Aktueller Preisstand</h3>
    <div class="tariff-grid">
      <div>
        <b>Umsatzbasiert</b>
        <strong>${SUMUP.payg.rate}</strong>
        <small>${SUMUP.payg.monthly}/Monat</small>
      </div>
      <div>
        <b>Zahlungen Plus</b>
        <strong>${SUMUP.plus.rate}</strong>
        <small>${SUMUP.plus.monthly}/Monat · ${SUMUP.plus.yearly}/Jahr</small>
      </div>
    </div>
    <p class="meta">
      Verifiziert am ${SUMUP.verified}. Nur deutsche SumUp-Konditionen
      (${SUMUP.sourceDomain}). ${SUMUP.plus.note}
    </p>
    <a class="official-link" href="${SUMUP.pricesUrl}" target="_blank" rel="noopener">
      ↗ Offizielle SumUp-Preise prüfen
    </a>
  </div>`;
}
...
function productHtml(name){
  ...
  return `<div class="product-card">
    <div>
      <b>Passender SumUp-Ansatz</b>
      <h4>${p.name}</h4>
      <div class="meta">${p.fit}</div>
    </div>
    <strong>${p.price}</strong>
    <a class="official-link" href="${p.url}" target="_blank" rel="noopener">
      ↗ Produktdetails bei SumUp
    </a>
  </div>`;
}
...
function nextQuestion(){
  if(A.solution==='Keine Kartenzahlung')
    return 'Was müsste sich bei Kosten, Bedienung oder Einfachheit ändern, damit Sie Kartenzahlung testen würden?';

  if(A.pain==='Kosten')
    return 'Geht es Ihnen eher um die einmaligen Anschaffungskosten oder um die laufenden Transaktionskosten?';

  if(A.pain==='Vertrag / Bindung')
    return 'Was stört Sie an der aktuellen Bindung konkret?';

  if(A.pain==='Mobilität')
    return 'Brauchen Sie die Kartenzahlung auch außerhalb eines festen Kassenplatzes?';

  if(A.pain==='Geschwindigkeit')
    return 'Geht es vor allem um die Dauer pro Zahlung oder um Wartezeiten bei Stoßzeiten?';

  if(A.pain==='Bedienung')
    return 'Was ist in der täglichen Bedienung aktuell unnötig kompliziert?';

  if(A.pain==='Technik')
    return 'Welche technische Situation tritt bei Ihnen am häufigsten auf?';

  if(A.pain==='Zahlungsarten')
    return 'Welche Zahlungsart fehlt Ihnen heute konkret?';

  if(A.pain==='Auszahlung / Abrechnung')
    return 'Was genau möchten Sie bei Auszahlung oder Abrechnung verbessern?';

  if(A.pain==='Support')
    return 'Was erwarten Sie von einem besseren Support konkret?';
...
function recommendationHtml(){
  ...
  return `<div class="recommendation">
    <div class="assistant-label">NE XARO · EMPFEHLUNG</div>
    <h4>${headline}</h4>
    <div class="script">${esc(text)}</div>
    ${productHtml(product)}
    ${fitHtml()}
  </div>`;
}
...
function recommendedProduct(){
  ...
  if(A.industry==='Gastronomie' && (A.pain==='Bedienung' || A.pain==='Geschwindigkeit'))
    return 'Terminal';
  return 'Solo';
}
...
function openLead(id){
  $('form').reset();
  $('id').value=id||'';
  $('dlgTitle').textContent=id?'Lead bearbeiten':'Neuer Lead';
  if(id){
    const l=S.leads.find(x=>x.id===id);
    ['company','industry','status','contact','phone','email','address','provider','terminal','product','priority','need','next','due','notes']
      .forEach(k=>$(k).value=l[k]||'');
  }
  $('dlg').showModal();
}

$('quick').onclick=()=>openLead();
$('add').onclick=()=>openLead();
...
function card(l){
  return `<div class="card lead">
    <div class="top">
      <div>
        <b>${esc(l.company)}</b>
        <div class="meta">${esc(l.industry)}${l.address?' · '+esc(l.address):''}</div>
      </div>
      <span class="pill">${esc(status(l.status))}</span>
    </div>
    ...
    ${l.phone?`<a href="tel:${esc(l.phone)}">📞 Anrufen</a>`:''}
    ${l.address?`<button onclick="nav('${encodeURIComponent(l.address)}')">🧭 Navigation</button>`:''}
    <button onclick="startForLead('${l.id}')">🤝 Assistant</button>
    <button onclick="visit('${l.id}')">📝 Besuch</button>
  </div>`;
}
...
function render(){
  ...
  const a=S.leads.filter(l=>
    (filter==='all'||l.status===filter) &&
    [l.company,l.contact,l.address,l.industry].join(' ').toLowerCase().includes(q)
  );

  $('list').innerHTML=a.length
    ? a.map(card).join('')
    : '<div class="card">Noch keine passenden Leads.</div>';

  const t=S.leads
    .filter(l=>l.due&&!['gewonnen','verloren'].includes(l.status))
    .sort((a,b)=>a.due.localeCompare(b.due));

  $('taskList').innerHTML=t.length
    ? t.map(card).join('')
    : '<div class="card">Keine offenen Follow-ups. Zeit für neue Besuche! 💪</div>';

  $('preview').innerHTML=t.slice(0,4)
    .map(l=>`<div class="card" style="margin:8px 0">
      <b>${esc(l.company)}</b>
      <div class="meta">${esc(l.next||'Follow-up')} · ${esc(l.due)}</div>
    </div>`)
    .join('')
    || '<div class="card">Keine offenen Follow-ups.</div>';

  $('areaList').innerHTML=S.leads.filter(l=>l.address).map(card).join('')
    || '<div class="card">Leads mit Adresse erscheinen hier.</div>';
...
function renderAssistant(){
  ...
  root.querySelectorAll('[data-a]').forEach(b=>
    b.onclick=()=>assistantAction(b.dataset.a,b.dataset.v||'')
  );
}
function step1(){
...
function step2(){
  ...
  ${A.solution?`<div class="tip">Auswahl: <b>${esc(A.solution)}</b></div>${sumupTariffHtml()}`:''}</div>`;
}
...
function step3(){
  ...
  if(A.solution==='Keine Kartenzahlung'){
    body=`<div class="question">Frage:</div>
    <div class="script">Darf ich fragen, warum Sie aktuell keine Kartenzahlung anbieten?</div>
    ...
    body=`<div class="question">Wie zufrieden ist der Betrieb mit der aktuellen Lösung?</div>
    <div class="choice-grid">${['Sehr zufrieden','Grundsätzlich zufrieden','Nicht zufrieden'].map(x=>`<button class="choice" data-a="satisfaction" data-v="${x}">${x}</button>`).join('')}</div>`;
  }

  return `<div class="assistant-card">
    <div class="assistant-label">Schritt 3 · Problem & Priorität</div>
    <h3>${A.solution==='Keine Kartenzahlung'?'Grund herausfinden':'Zufriedenheit prüfen'}</h3>
    ${body}

    ${A.satisfaction?`
      <div class="question">Nächste Frage:</div>
      <div class="script">${A.satisfaction==='Nicht zufrieden'
        ?'Was stört Sie denn momentan am meisten?'
        :'Wenn Sie eine Sache sofort verbessern könnten – welche wäre das?'}</div>
    ...
    ${A.pain?`
      <div class="tip">Bedarf erfasst: <b>${esc(A.pain)}</b></div>
      ${recommendationHtml()}
      <div class="assistant-actions">
        <button class="primary" data-a="next3">Einwand vorbereiten →</button>
      </div>
    `:''}
  </div>`;
}
...
function step4(){
  ...
  if(ob){
    return `<div class="assistant-card">
      ...
      <div class="script">
        <b>Antwort:</b><br>${esc(objections[ob].say)}
      </div>

      <div class="question">Danach fragen:</div>
      <div class="script">${esc(objections[ob].ask)}</div>

      ${A.pain?`
        <div class="tip">
          <b>Zurück zum Bedarf:</b> ${esc(nextQuestion())}
        </div>
      `:''}

      <div class="assistant-actions">
        <button class="primary" data-a="close">Zum Abschluss →</button>
        <button data-a="clearObjection">Anderen Einwand</button>
      </div>
    </div>`;
  }
  ...
  return `<div class="assistant-card">
    ...
    <div class="choice-grid two">
      ${Object.keys(objections).map(x=>`<button class="choice" data-a="objection" data-v="${x}">${x}</button>`).join('')}
    </div>

    <div class="assistant-actions">
      <button class="primary" data-a="close">Kein Einwand – zum Abschluss →</button>
    </div>
  </div>`;
}
...
function step5(){
  return `<div class="assistant-card result">
    ...
    <div class="choice-grid">
      ${['Abschluss heute','Rückruf vereinbaren','Termin vereinbaren','Unterlagen / Info senden','Entscheider kontaktieren','Kein Interesse'].map(x=>`<button class="choice" data-a="finish" data-v="${x}">${x}</button>`).join('')}
    </div>

    ${A.timing?`
      <div class="saved">✓ ${esc(A.timing)}${A.saved?' · CRM gespeichert':''}</div>
    `:''}
...
function saveAssistantToCRM(action){
  ...
  if(['Rückruf vereinbaren','Termin vereinbaren','Entscheider kontaktieren'].includes(action)){
...
$('locate').onclick=()=>{
  if(navigator.geolocation){
    navigator.geolocation.getCurrentPosition(
      ...
      ()=>$('locStatus').textContent='Standortzugriff nicht erlaubt.'
    );
  }else{
...
$('csv').onclick=()=>{
  const c=['company','industry','status','contact','phone','email','address','provider','terminal','product','priority','need','next','due','notes'];

  dl(
    'nexaro-leads.csv',
    '\ufeff'+[
      c.join(';'),
      ...S.leads.map(l=>c.map(k=>`"${String(l[k]||'').replaceAll('"','""')}"`).join(';'))
    ].join('\n'),
    'text/csv'
  );
};
